...
This starts the full development stack: the ingestion watcher server, TypeScript asset compilation, and the Eleventy dev server with live reload. Drag-and-drop uploads work out of the box.

If you only need the static site without the watcher server:
...
| `npm run dev` | Full dev stack: watcher + assets + Eleventy serve |
| `npm run dev:static` | Dev server without the ingestion watcher |
